...
const ShopCart: React.FC = () => {
  const { cart, totalPrice, dispatch } = useCartContext();
      ...
      <div className={`cart-wrapper ${isOpen ? "open" : ""}`}>
        ...
        <div className="cart-wrapper__body">
          {cart.length <= 0 ? (
            ...
            cart.map((item: IProducts, i: number) => (
              <ShopCartItem key={i} item={item} />
            ))
          )}
        </div>
        <div className="cart-wrapper__footer">
          <h2 className="cart-wrapper__footer__total-price">
            Total price : {totalPrice} $
          </h2>
          <Button
            onClick={() => navigate("/cart")}
            className="cart-wrapper__footer__button"
          >
            check out
          </Button>
        </div>
