...
const Product: React.FC = () => {
  const RelatedProduct = useMemo(
    () => lazy(() => import("../../Layout/RelatedProduct/RelatedProduct")),
    []
  );
  ...
  const handleAddToCart = () => {
    if (+values.quantity >= 1 && product?.inStock) {
      ...
      errorToast(
        "cant add product to cart",
        "make sure put number in quantity input"
      );
    }
  };
    ...
    try {
      ...
      const fethProduct = async () => {
        ...
        if (!data.exists()) {
          ...
          errorToast(
            "product not found",
            "make sure enter correct product id in url"
          );
        }
      ...
      errorToast(
        ...
        "make sure you have accses to internet "
      );
      console.log(error);
    }
  }, [id]);

  const { handleChange, handleSubmit, values } = useForm(handleAddToCart, {
    quantity: 1,
  });
  if (!product) {
    return (
  ...
  const isAddToCart = cart.find((item) => item.id === product.id)
    ? true
    : false;
  return (
    <Container>
      ...
      <div className="product-page">
        ...
        <div className="product-page__content">
          ...
          <span className="product-page__content__date">
            {new Timestamp(
              product.timeStamp.seconds,
              product.timeStamp.nanoseconds
            )
              .toDate()
              .toDateString()}
          </span>
          ...
          <div className="product-page__content__prices">
            {product.discountPercent > 0 && (
              <del className="product-page__content__non-discount-price">
                $ {product.price}
              </del>
            )}
            ...
            <h4 className="product-page__status__stock">
              ...
              {product.inStock ? (
                <span className="product-page__status__stock-text">
                  in stock{" "}
                  <TiTick className="product-page__status__stock-icon" />
                </span>
              ) : (
                <span className="product-page__status__stock-text">
                  out of stock{" "}
                  <RiCloseFill className="product-page__status__out-stock-icon" />
                </span>
              )}
            </h4>
            <h4 className="product-page__status-text">
      ...
      <div className="product-page__sugested-product">
        <h4 className="product-page__sugested-product__title">
          related producs
        </h4>
